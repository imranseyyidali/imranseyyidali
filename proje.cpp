...
public:
    ...
    void returnayakkabi(int index) {
        ...
        void deleteayakkabi(int index) {
        if (index >= 0 && index < ayakkabis.size()) {
            cout << "Ayakkabi silindi: " << ayakkabis[index].brand << " " << ayakkabis[index].model << endl;
            ayakkabis.erase(ayakkabis.begin() + index); 
        } else {
            cout << "Geçersiz ayakkabi indeksi!" << endl;
        }
    }
...
int main() {
    ...
    while (true) {
        cout << "\nBir komut girin (ekle, goster, kirala, teslim, sil, exit): ";
        cin >> command;
        ...
        if (command == "ekle") {
            ...
            int index;
            cout << "Kiralanacak ayakkabi indeksi: ";
            cin >> index;
            system.rentayakkabi(index);
        } else if (command == "teslim") {
            int index;
            cout << "Teslim al?nacak ayakkabi indeksi: ";
            cin >> index;
            system.returnayakkabi(index);
        } else if (command == "sil") { 
            int index;
            cout << "Silinecek ayakkabi indeksi: ";
            cin >> index;
            system.deleteayakkabi(index);
